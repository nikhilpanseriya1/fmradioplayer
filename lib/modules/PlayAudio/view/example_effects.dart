...
class AudioEffects extends StatefulWidget {
  ...
  AudioEffects({Key? key, this.flag, this.audio}) : super(key: key);

  @override
  MyAppState createState() => MyAppState();
}

class MyAppState extends State<AudioEffects> with WidgetsBindingObserver {
  ...
  Future<void> _init() async {
    ...
    try {
      await _player.setAudioSource(widget.flag == true
          ? AudioSource.uri(Uri.parse(widget.audio ?? ''))
          : AudioSource.uri(Uri.file(widget.audio ?? '')));
    } catch (e) {
    ...
    return StreamBuilder<double>(
      ...
      builder: (context, snapshot) {
        ...
        return Slider(
          ...
          label: 'foo',
          activeColor: colorRed,
          inactiveColor: colorRed.withOpacity(0.10),
        );
      },
    );
  }
